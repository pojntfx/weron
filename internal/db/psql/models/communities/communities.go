...
func Communities(mods ...qm.QueryMod) communityQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"communities\".*"})
	}

	return communityQuery{q}
}
...
func (o *Community) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(communityAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(communityPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert communities, could not build conflict column list")
			}

			conflict = make([]string, len(communityPrimaryKeyColumns))
			copy(conflict, communityPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"communities\"", updateOnConflict, ret, update, conflict, insert, opts...)
